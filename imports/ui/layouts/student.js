...
import { Onboarding } from '../../api/Onboarding/Onboarding.js';
  ...
  render() {
    ...
    <Row type="flex" justify="space-around" align="middle"  style={{zIndex: 1000, height: 55, position: 'fixed', top: 0, right: 0, left: 0, paddingLeft: 20, color: '#fff', backgroundColor: '#da5347', borderBottom: '1px solid #e9e9e9'}}>
        <Col span='19'>
      	...
      	<Col span='3'>
      		<Menu onClick={this.handleClick} selectedKeys={[this.props.pathname]} mode="horizontal" style={{fontWeight: 300, color: '#fff', fontSize: 17, borderBottom: '0px solid transparent', backgroundColor: '#da5347',}}>
  ...
  componentWillMount(){
    ...
    return (
    	<div>
	        ...
	        <div style={{marginLeft: "265px", padding: "100px 0px"}}>
...
const composer = (params, onData) => {
  const subscription = Meteor.subscribe('onboarding');
  if (subscription.ready()) {
    const onboarding = Onboarding.findOne({ownerId: Meteor.userId()});
    onData(null, { onboarding });
  }
};

export default composeWithTracker(composer, Loading)(StudentLayout);
